...
def print_plan(config: dict):
    ...
    print(f"    Rollback if:    error_rate > {rb['error_rate_threshold']*100:.1f}% over {rb['window']}s")
    print(f"    Poll interval:  {rb['poll_interval']}s")
    print()
    print("  ─── Execution Steps ──────────────────────────────")
    print()
    print(f"    1 │ Start Green container on port {ports['green']}")
    print(f"    2 │ Health check Green at :{ports['green']}{hc['path']}")
    print(f"    3 │ Switch Nginx traffic  Blue → Green")
    print(f"    4 │ Monitor error rate for {rb['window']}s")
    print(f"    5 │ If error rate > {rb['error_rate_threshold']*100:.1f}%  → auto-rollback to Blue")
    print(f"    6 │ If clean              → promote Green as production")
    print()
    print("  ─── No changes made ──────────────────────────────")
    print()
    print("    Run 'anchor apply' to execute this plan.")
    print()
